...
4a. **Validate the manifest against the kit source.** Before delivering, run:

   ```
   node scripts/validate-manifest.mjs
   ```

   It re-walks `src/components/ui/**` independently of your extractor — a
   check that shared your file discovery could not catch a discovery bug —
   and fails on: a `.tsx` on disk with no manifest entry, an entry naming a
   file that does not exist, a missing or invalid `role`/`partOf`, a
   `defaultVariants` value outside its axis, a file that calls `cva()` whose
   entry has empty variants, an unresolved `var()` left in `tokens.json`, an
   `extract-manifest.mjs` that does not use `loadTsMorph()`, and a modified
   kit lockfile. Its WARNs — an axis visible in a `cva()` block but absent
   from the manifest, an unresolvable semantic alias — each need you to open
   the file and either fix the extractor or explain the warning in the
   report. Paste the full output into `data/extraction-report.md` and the MR
   description.

   Wire it into the `uijourney:extract` npm script so extraction cannot
   succeed while producing an invalid manifest.

4b. **Prove the manifest did not shrink.** Before delivering, run:
...
4. Add an npm script `uijourney:extract` running all three in order, and a
   **GitLab CI** job in `.gitlab-ci.yml` (extend the existing pipeline;
   ...
   `src/components/ui/**` or the token CSS, runs
   `node scripts/validate-manifest.mjs`, and fails if the committed
   `data/*.json` is stale (`git diff --exit-code data/`). The staleness check
   is also what proves extraction is reproducible from a cold checkout.
...
- `node scripts/validate-manifest.mjs` exits zero, and every WARN it prints
  is explained in the extraction report.
